...
export async function getServerPageSession
    (options: Omit<Apollo.QueryOptions<Types.SessionQueryVariables>, 'query'>, ctx? :any ){
        const apolloClient = getApolloClient(ctx);
        
        const data = await apolloClient.query<Types.SessionQuery>({ ...options, query: Operations.SessionDocument });
...
export const useSession = (
  optionsFunc?: (router: NextRouter)=> QueryHookOptions<Types.SessionQuery, Types.SessionQueryVariables>) => {
  const router = useRouter();
  const options = optionsFunc ? optionsFunc(router) : {};
  return useQuery(Operations.SessionDocument, options);
};
export type PageSessionComp = React.FC<{data?: Types.SessionQuery, error?: Apollo.ApolloError}>;
